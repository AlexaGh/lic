...
class SummarizationModel(object):
  ...
  def _add_seq2seq(self):
    ...
    #C:\pointer - generator - master - pltrdy\pointer - generator - master
    data_directory = 'C:/pointer-generator-master-pltrdy/pointer-generator-master'
    if not os.path.isdir(data_directory):
        os.path.makedirs(data_directory)

    glove_weights_file_path = os.path.join(data_directory, 'glove.6B.100d.txt')
    ...
    features = {}
    features['word_indices'] = tf.placeholder(tf.int32, [hps.batch_size, 50])
    features['word_indices'] = vocab
    ...
    with tf.variable_scope('seq2seq'):
      ...
      with tf.variable_scope('embedding'):
        embedding = tf.get_variable('embedding', shape=(VOCAB_SIZE, EMBEDDING_DIMENSION), dtype=tf.float32, initializer=glove_weights_initializer, trainable=False)
        if hps.mode=="train": self._add_emb_vis(embedding) # add to tensorboard
        emb_enc_inputs = tf.nn.embedding_lookup(embedding,  self._enc_batch) # tensor with shape (batch_size, max_enc_steps, emb_size)  (embedding, self._enc_batch)list(word_to_idx.values())).eval()
